...
impl<'a> MarkdownRenderEngine<'a> {
    ...
    fn parse_markdown<TFrontmatter: DeserializeOwned>(
        &'a self,
        markdown: String,
        ...
        Ok(ParsedMarkdown {
            frontmatter: frontmatter.context("Failed to parse frontmatter")?,
    ...
    fn build_index_html(&'a self, posts: Vec<PostMetadata>) -> Result<()> {
        ...
        let html = Page {
            title: result.frontmatter.title.clone(),
    ...
    fn build_pages(&'a self) -> Result<()> {
        ...
        for entry in WalkDir::new(pages_dir)
            .into_iter()
            .filter_map(|e| e.ok())
            .filter(|e| e.file_type().is_file() && e.path() != Path::new("index.md"))
        {
            let path = entry.path();
